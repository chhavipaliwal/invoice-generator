...
export function Invoice1(
    ...
    <div class="flex flex-col p-4 sm:p-10 bg-white shadow-md rounded-xl">
        ...
        <div class="w-full max-w-2xl ml-auto sm:text-end space-y-2">
          <div class="grid grid-cols-3">
            <dl class="grid col-start-3 sm:grid-cols-2 gap-x-3">
              ...
              <dd class="col-span-1 text-gray-500">${formatCurrency(data.summary?.subtotal || 0)}</dd>
            </dl>

            ${data.summary?.shipping?.isShipping ? `
            <dl class="grid col-start-3 sm:grid-cols-2 gap-x-3">
              <dt class="col-span-1 font-semibold text-gray-800">Shipping:</dt>
              <dd class="col-span-1 text-gray-500">+ ${formatCurrency(data.summary?.totalShipping || 0)}</dd>
            </dl>
            ` : ''}
            ...
            <dl class="grid col-start-3 sm:grid-cols-2 gap-x-3">
              ...
              <dd class="col-span-1 text-gray-500">+ ${formatCurrency(data.summary.totalTax || 0)}</dd>
            </dl>
            ` : ''}
            ...
            ${data.summary?.discount?.isDiscount ? `
                <dl class="grid col-start-3 sm:grid-cols-2 gap-x-3">
                  ...
                  <dd class="col-span-1 text-success-500">- ${formatCurrency(data.summary?.totalDiscount || 0)}</dd>
                </dl>
                ` : ''}

             <dl class="grid col-start-3 sm:grid-cols-2 gap-x-3">
              <dt class="col-span-1 font-semibold text-gray-800">Total:</dt>
              <dd class="col-span-1 border-t border-gray-200 font-medium text-gray-900">${formatCurrency(data.summary?.total || 0)}</dd>
            </dl>
          </div>
        </div>
      </div>
      ...
      <div class="mt-8 sm:mt-12">
        ...
        <div class="mt-2">
          <p class="block text-sm font-medium text-gray-800">${data.from?.email || ""}</p>
          <p class="block text-sm font-medium text-gray-800">${data.from?.phonenumber || ""}</p>
        </div>
      </div>
