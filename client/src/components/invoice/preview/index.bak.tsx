...
export default function Preview() {
  ...
  return (
    <div className="bg-gray-100 text-black flex justify-center rounded-large p-4">
      <div className=" p-8 rounded-lg shadow-lg w-full max-w-2xl">
        ...
        <table className="w-full table mt-6 border-collapse border border-gray-300 text-black">
          ...
          <tbody className="divide-y divide-gray-300">
            {formik.values.lineItems?.map((item) => (
              <tr key={item.id} className="table-row divide-x divide-gray-300">
                <td className="h-full max-w-sm px-4 py-2 flex flex-col gap-2">
                  <span>{item.name}</span>
                  {item.description && item.description.length > 384 ? (
                    <>
                      <br />
        ...
        <div className="mt-4 text-right">
          <p className="font-semibold">
            ...
            <span className="text-gray-700">
              ...
              {formik.values?.summary?.subtotal || 0}
            </span>
          </p>
          {formik.values?.summary?.discount?.amount &&
            formik.values?.summary?.discount?.amount > 0 && (
              <p className="font-semibold">
                Discount:{" "}
                <span className="text-gray-700">
                  {formik.values?.summary?.discount?.type === "percentage"
                    ? `${formik.values?.summary?.discount?.amount}%`
                    : `${formik.values?.currency}${formik.values?.summary?.discount?.amount}`}
                </span>
              </p>
            )}
